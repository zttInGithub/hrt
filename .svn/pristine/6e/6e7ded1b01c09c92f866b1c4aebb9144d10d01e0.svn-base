...
<script type="text/javascript">
	...
	//初始化datagrid
	$(function() {
		$('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid({
			url :'${ctx}/sysAdmin/merchant_listMicroMerchantInfoWJoinSyt.action',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [[{
				...
				formatter : function(value,row,index) {
					if(row.approveStatus == 'W'){
						...
						return  '<img src="${ctx}/images/query_search.png" title="查看明细" style="cursor:pointer;" onclick="sysAdmin_merchantmicrojoin_queryFun('+index+','+row.bmid+',\''+row.mid+'\')"/>&nbsp;&nbsp'+
						 '<img src="${ctx}/images/frame_pencil.png" title="查看相同营业执照号" style="cursor:pointer;" onclick="sysAdmin_merchantBnoRname_queryFun('+row.bno+')"/>&nbsp;&nbsp'+
			...
			toolbar:[{
					...
					iconCls:'icon-start',
					handler:function(){
						sysAdmin_merchantbackAll_editFun();
					}
			}]  
		});
	});
	...
	function sysAdmin_merchantmicrojoin_editFun(){
		var checkedItems = $('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid('getChecked');
		var names = [];
		var result = true;
					...
					$('#sysAdmin_merchantmicrojoin_editForm').form('submit', {
						...
						success:function(data) {
							...
							if (result.sessionExpire) {
				    			...
				    			if (result.success) {
				    				$('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid('unselectAll');
				    				$('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid('reload');
	...
	function sysAdmin_merchantmicrojoin_queryFun(index,bmid,mid){
		$('<div id="sysAdmin_merchantmicrojoin_queryDialog"/>').dialog({
		    ...
		    modal: true,
		    onLoad:function() {
		    	var rows = $('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid('getRows');
				var row = rows[index];
				// debugger;
				if(!row.scanRate){
					row.scanRate="0.38";
				...
				if(!row.secondRate){
					row.scanRateAll=row.scanRate1+"+3";
				}else{
					row.scanRateAll=row.scanRate1+"+"+row.secondRate;
				}
				// console.log(row);
	...
	function sysAdmin_merchantbackAll_editFun(){
		var checkedItems = $('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid('getChecked');
		var names = [];
		$.each(checkedItems, function(index, item){
			names.push(item.bmid);
	...
	function sysAdmin_merchantmicroAuid_searchFun20() {
		$('#sysAdmin_merchantmicrojoinYsb_datagrid').datagrid('load', serializeObject($('#sysAdmin_merchantmicroAuid_searchYsbForm')));  
	} 

	//清除表单内容
	function sysAdmin_merchantmicroAuid_cleanFun20() {
		$('#sysAdmin_merchantmicroAuid_searchYsbForm input').val('');
	}
	
</script>
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:80px; overflow: hidden; padding-top:10px;">
		<form id="sysAdmin_merchantmicroAuid_searchYsbForm" style="padding-left:10%">
	...
	<div data-options="region:'center', border:false" style="overflow: hidden;">  
		<table id="sysAdmin_merchantmicrojoinYsb_datagrid" style="overflow: hidden;"></table>
	</div>
</div>