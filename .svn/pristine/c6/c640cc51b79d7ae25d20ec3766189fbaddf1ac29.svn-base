...
<script type="text/javascript">
	...
	//初始化datagrid
	$(function() {
		$('#sysAdmin_10497_datagrid').datagrid({
			...
			/**,{
				...
				align : 'center',
				formatter : function(value,row,index) {
					var rowData = $('#sysAdmin_10497_datagrid').datagrid('getData').rows[index];
					if(rowData.qrUrl!=null&&rowData.qrUpload!=null){
						return '<img src="${ctx}/images/query_search.png" title="查看二维码" style="cursor:pointer;" onclick="sysAdmin_10497_queryFun('+index+')"/>&nbsp;&nbsp';
					}
			}
			}**/]],
			toolbar:[{
				...
				iconCls:'icon-add',
				handler:function(){
					sysAdmin_merchantterminalinfoSelected10497_create();
				}
			},{
				id:'btn_add',
				text:'分配邀请码',
				iconCls:'icon-add',
				handler:function(){
					sysAdmin_merchantterminalinfoSelected10497_fenpei();
				}
			},{
				id:'btn_run',
				text:'导出查询所有',
				...
				handler:function(){
					sysAdmin_merchantterminalinfoSelected10497_exportFunAll();
				}
			}/**,{
				id:'btn_run',
				text:'勾选导出',
				iconCls:'icon-query-export',
				handler:function(){
					sysAdmin_merchantterminalinfoSelected10497_exportFun();
				}
			}**/]
		});
	});

	function sysAdmin_merchantterminalinfoSelected10497_exportFunAll() {
		$('#sysAdmin_10497_searchForm').form('submit',{
			    		url:'${ctx}/sysAdmin/qrInvitationInfo_exportMerQRAll.action'
			    	});
	}
	
	function sysAdmin_merchantterminalinfoSelected10497_fenpei(){
		var checkedItems = $('#sysAdmin_10497_datagrid').datagrid('getChecked');
		var names = [];
		var result = true;
			...
			if (result) {
				$('<div id="sysAdmin_fen494_datagrid"/>').dialog({
					...
					buttons:[{
							...
							$('#fenpeiInfo').form('submit', {
								...
								success:function(data) {
									...
									if (result.sessionExpire) {
						    			...
						    			if (result.success) {
						    				$('#sysAdmin_10497_datagrid').datagrid('unselectAll');
						    				$('#sysAdmin_10497_datagrid').datagrid('reload');
						...
						iconCls:'icon-cancel',
						handler:function() {
							$('#sysAdmin_10496_datagrid').dialog('destroy');
						}
					}],
					onClose:function() {
						$(this).dialog('destroy');
	...
	function sysAdmin_merchantterminalinfoSelected10497_create(){
	...
	function sysAdmin_merchantterminalinfo10497_exportFun() {
		$('#sysAdmin_10497_searchForm').form('submit',{
			    		url:'${ctx}/sysAdmin/merchant_updateMerchantWithQR.action'
			    	});
	}
	
	function sysAdmin_merchantterminalinfoSelected10497_exportFun() {
		var checkedItems = $('#sysAdmin_10497_datagrid').datagrid('getChecked');
		var names = [];
		$.each(checkedItems, function(index, item){
		...
		$("#bmids_10497").val(bmids);
		$('#sysAdmin_10497_searchForm').form('submit',{
			url:'${ctx}/sysAdmin/merchant_updateMerchantWithQR.action',
			success:function(data){
			    $('#sysAdmin_10497_datagrid').datagrid('load', serializeObject($('#sysAdmin_10497_searchForm'))); 
			    var result = $.parseJSON(data);
			    $.messager.show({
					title : '提示',
					msg : result.msg
	...
	//表单查询
	function sysAdmin_merchantAuthenticity_searchFun10497() {
		$('#sysAdmin_10497_datagrid').datagrid('load', serializeObject($('#sysAdmin_10497_searchForm'))); 
	}

	//清除表单内容
	function sysAdmin_merchantAuthenticity_cleanFun10497() {
		$('#sysAdmin_10497_searchForm input').val('');
		$('#sysAdmin_10497_searchForm #status').val('');
	}
...
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:100px; overflow: hidden; padding-top:15px;">
		<form id="sysAdmin_10497_searchForm" style="padding-left:2%" method="post">
		<input type="hidden" id="bmids_10497" name="bmids" />
			<table class="tableForm" >
				...
				<tr>
					...
					<td colspan="5" style="text-align: center;">
						...
						onclick="sysAdmin_merchantAuthenticity_searchFun10497();">查询</a> &nbsp;
						<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" 
						onclick="sysAdmin_merchantAuthenticity_cleanFun10497();">清空</a>	
					</td>
				</tr>
			</table>
		</form>
    ...
    <div data-options="region:'center', border:false" style="overflow: hidden;">  
      <table id="sysAdmin_10497_datagrid" style="overflow: hidden;"></table>
    </div> 
</div>