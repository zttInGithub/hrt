...
<script type="text/javascript"> 
	$(function(){
		var bmutID=<%=request.getParameter("BMUTID")%>;
			$.ajax({
				url:'${ctx}/sysAdmin/merchantUpdateTer_queryDetail.action',
	    		dataType:"json",  
	    		type:"post",
	    		data:{bmutID:bmutID},
		...
		//商户基本信息工单申请图片预览
		$("#sysAdmin_10951_from #snImgFile").uploadPreview({ Img: "ImgPr1", Width: 120, Height: 120 });
	});
	...
	function showBigImg(img){
		...
		$('#sysAdmin_10951_from').after(imgDialog);
		$("#img").attr("src",img);
...
<form id="sysAdmin_10951_from" style="padding-left:2%;padding-top:1%" method="post" enctype="multipart/form-data">
	<table class="table">
		<tr>
   			<th>机构号：</th>
   			<td>
   				<input type="text" name="UNNO" style="width:200px;" class="easyui-validatebox" data-options="required:true,validType:'spaceValidator'" readonly="readonly" maxlength="50"/>
   			</td>
			<th>商户号：</th>
   			<td>
