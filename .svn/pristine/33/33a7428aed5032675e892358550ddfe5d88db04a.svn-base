...
<script type="text/javascript">
	//初始化datagrid
	$(function() {
		$('#sysAdmin_merchantZK_datagrid').datagrid({
			...
			toolbar:[/**{
					...
					iconCls:'icon-add',
					handler:function(){
						sysAdmin_merchant1310410_addFun();
					}
				}/**,{
					id:'btn_add',
					text:'优惠商户签约',
	...
	//企业商户签约（费改）
	function sysAdmin_merchant1310410_addFun(){
				$('<div id="sysAdmin_addMerchant13_datagrid"/>').dialog({
		    		...
		    		maximizable:true,
				    closed: false,
				    href: '${ctx}/biz/bill/merchant/10425_1.jsp',  
				    modal: true,
				    buttons:[{
				    	text:'确认',
				    	iconCls:'icon-ok',
	...
	**/
	function sysAdmin_merchant12_editFun(index,isForeign,merchantType,bmid,accType){
		$('<div id="sysAdmin_merchant_editDialog"/>').dialog({
		    ...
		    modal: true,
		    onLoad:function() {
		    	var rows = $('#sysAdmin_merchantZK_datagrid').datagrid('getRows');
				var row = rows[index];
	...
	function sysAdmin_merchantZK_searchFun10() {
		$('#sysAdmin_merchantZK_datagrid').datagrid('load', serializeObject($('#sysAdmin_merchantZK_searchForm'))); 
	}
...
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:80px; overflow: hidden; padding-top:10px;">
		<form id="sysAdmin_merchantZK_searchForm" style="padding-left:10%">
 	...
 	<div data-options="region:'center', border:false" style="overflow: hidden;">  
		<table id="sysAdmin_merchantZK_datagrid" style="overflow: hidden;"></table> 
	</div>
</div>
