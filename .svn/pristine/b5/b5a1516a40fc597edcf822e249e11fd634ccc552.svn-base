...
<script type="text/javascript">
	...
	//初始化datagrid
	$(function() {
		$('#sysAdmin_merchantjoin429_datagrid').datagrid({
			url :'${ctx}/sysAdmin/merchant_listMerchantInfoWJoin2.action',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [[{
				...
				formatter : function(value,row,index) {
					if(row.approveStatus == 'C'&&row.isM35==6){
						return '<img src="${ctx}/images/query_search.png" title="查看明细" style="cursor:pointer;" onclick="sysAdmin_merchantjoin429_queryFun('+index+')"/>&nbsp;&nbsp'+
						'<input type="button" value="调整限额" onclick="sysAdmin_merchantY409_limit('+index+')"/>';
					}if(row.approveStatus == 'C'){
						return '<img src="${ctx}/images/frame_pencil.png" title="查看相同营业执照号商户" style="cursor:pointer;" onclick="sysAdmin_merchantBnoRname_queryFun('+row.bno+')"/>&nbsp;&nbsp'+
						'<input type="button" value="审批通过" onclick="sysAdmin_merchantY_editFun('+row.bmid+')"/>';
					}else{
						return '<img src="${ctx}/images/query_search.png" title="查看明细" style="cursor:pointer;" onclick="sysAdmin_merchantjoin429_queryFun('+index+')"/>&nbsp;&nbsp'+
						'<img src="${ctx}/images/frame_pencil.png" title="查看相同营业执照号商户" style="cursor:pointer;" onclick="sysAdmin_merchantBnoRname_queryFun('+row.bno+')"/>';
					}
				}
			}]],
			toolbar:[{
					...
					iconCls:'icon-start',
					handler:function(){
						sysAdmin_merchantjoin429_editFun();
					}
				},{
					id:'btn_add',
					text:'退回',
					iconCls:'icon-close',
					handler:function(){
						sysAdmin_merchantjoin429k_editFun();
					}
				},{
					id:'btn_add',
					text:'审批通过',
					iconCls:'icon-start',
					handler:function(){
						sysAdmin_merchantjoin429Y_editFun();
					}
				}]  
		});
	});
		        ...
		        $.ajax({
		        	...
		        	success: function(data, textStatus) {
			        	if (data.success) {
				        	...
				        	$('#sysAdmin_merchantjoin429_datagrid').datagrid('reload');
				        } else {
				        	$.messager.alert('提示', data.msg);
				        	$('#sysAdmin_merchantjoin429_datagrid').datagrid('unselectAll');
					    }
			        },
	...
	function sysAdmin_merchantY409_limit(index){
		var rows = $('#sysAdmin_merchantjoin429_datagrid').datagrid('getRows');
		var row = rows[index];
		$('<div id="sysAdmin_merchantjoin_editDialog"/>').dialog({
			...
			buttons:[{
				...
				iconCls:'icon-ok',
				handler:function() {
					var validator = $('#sysAdmin_merchantjoin429_datagrid').form('validate');
		    		if(validator){
		    			$.messager.progress();
		    		}
				...
				handler:function() {
					...
					$('#sysAdmin_merchant10509_editForm').form('submit', {
						url:'${ctx}/sysAdmin/merchant_updateJHMerchantInfoK.action?mid='+row.mid,
						success:function(data) {
							$.messager.progress('close');
							var result = $.parseJSON(data);
							if (result.sessionExpire) {
	...
	function sysAdmin_merchantjoin429_queryFun(index){
		var rows = $('#sysAdmin_merchantjoin429_datagrid').datagrid('getRows');
		var row  = rows[index];
		var bmid = row.bmid;
		var mid  = row.mid;
		$('<div id="sysAdmin_merchantjoin_queryDialog"/>').dialog({
		    ...
		    height:550, 
		    closed: false,
		    href: '${ctx}/biz/bill/merchant/10481.jsp?bmid='+bmid+'&mid="'+mid+'"',
		    modal: true,
		    onLoad:function() {
	...
	function sysAdmin_merchantjoin429_editFun(){
		var checkedItems = $('#sysAdmin_merchantjoin429_datagrid').datagrid('getChecked');
		var names = [];
		var result = true;
	...
	//二次审批 
	function sysAdmin_merchantjoin429Y_editFun(){
		var checkedItems = $('#sysAdmin_merchantjoin429_datagrid').datagrid('getChecked');
		var names = [];
		var result = true;
		$.each(checkedItems, function(index, item){
	...
	function sysAdmin_merchantjoin429k_editFun(){
		var checkedItems = $('#sysAdmin_merchantjoin429_datagrid').datagrid('getChecked');
		var names = [];
		var result = true;
		$.each(checkedItems, function(index, item){
	...
	function sysAdmin_merchantBnoRname_queryFun(bno){
		...
		//表单查询
	function sysAdmin_merchantAuid429_searchFun20() {
		$('#sysAdmin_merchantjoin429_datagrid').datagrid('load', serializeObject($('#sysAdmin_merchantAuid429_searchForm'))); 
	}

	//清除表单内容
	function sysAdmin_merchantAuid429_cleanFun20() {
		$('#sysAdmin_merchantAuid429_searchForm input').val('');
	}
	...
	<div data-options="region:'north',border:false" style="height:80px; overflow: hidden; padding-top:25px;">
		<form id="sysAdmin_merchantAuid429_searchForm" style="padding-left:3%">
			<table class="tableForm" >
				<tr>
					<th>机构名称</th>
					<td><input name="unitName" style="width: 100px;" /></td>
					<th>&nbsp;&nbsp;&nbsp;商户编号</th>
					...
					<th>&nbsp;&nbsp;&nbsp;商户状态</th>
					<td>
						<select name="isM35" class="easyui-combobox" data-options="panelHeight:'auto',editable:false"  style="width:70px;">
					...
					<td colspan="5" style="text-align: center;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						...
						onclick="sysAdmin_merchantAuid429_searchFun20();">查询</a> &nbsp;
						<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" 
						onclick="sysAdmin_merchantAuid429_cleanFun20();">清空</a>	
					</td>
				</tr>
			</table>
		</form>
	...
	<div data-options="region:'center', border:false" style="overflow: hidden;">  
		<table id="sysAdmin_merchantjoin429_datagrid" style="overflow: hidden;"></table>
	</div>
