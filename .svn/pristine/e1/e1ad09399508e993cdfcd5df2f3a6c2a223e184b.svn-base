...
<script type="text/javascript">
		...
		+'<table class="table">'
			...
			+'<tbody id="00312_tab2_body">'
					...
					+'<tr id="00312T19">'
						...
						+'</td>'
					+'</tr>'
					// +'<tr id="00312TRebateType">'
					// 	+'<th style="text-align: center;">活动</th>'
					// 	+'<td style="text-align: center;" colspan="9"><a id="" onclick="sysAdmin_00312_addRebate()" class="l-btn" href="javascript:void(0)"><span class="l-btn-left"><span class="l-btn-text icon-add" style="padding-left: 20px;">添加活动成本</span></span></a></td>'
					// +'</tr>'
			+'</tbody>'
		+'</table>'
	+'</fieldset>'
			...
			dataType:"json",  
	   		...
	   		success:function(data){
	   			if(data.success){
	   				...
	   				if(keys.length != 0){
	   					$("#flag_00312").val(1);
	   					$("#00312_hrtCostForm").append(html);
	   					$.each($("#00312_hrtCostForm").find('input'), function(index) {
		   					var id = $(this).attr('id');
	   		    		...
	   		    		var _trs = $("#00312_tab2_body").children("tr");
	   		    		for(var i = 1; i < _trs.length+1; i ++){
	   		    			if(i == 3||i == 17||i == 18||i == 19){
	   		    				continue;
	   		    			}
	   		    			$('#00312T'+i+'').find("input[name='debitRate']").bind('blur',function(){
	   		    	            var _debitRate_value = $(this).val();
	   		    	            $(this).parent().parent().find("input[name='creditRate']").val(_debitRate_value);
	   		    	   		});
	   		    		}
	   					/* $('#00312T1').find("input[name='debitRate']").bind('blur',function(){
	   			            var _debitRate_value = $('#00312T1').find("input[name='debitRate']").val();
	   			            $('#00312T1').find("input[name='creditRate']").val(_debitRate_value)
	   			   		}); */
	   		    		//去加载活动成本
		   				sysAdmin_00312_queryRebate(buid);
	   				}else{
							$("#00312_ACT_TAB").remove();
						}
	   			}
	   		}
		});
	...
	function sysAdmin_00312_queryRebate(buid){
		//加载活动成本
		$.ajax({
	   			...
	   			if(data.success){
	   				...
	   				if(obj.length != 0){
	   					for(var k=0;k<obj.length;k++){
	   						...
	   						var tr_list = $("#00312_ACT_TR").children("tr");
		   					var id = tr_list.length;

	   						/*var html = '<tr id="00312T'+id+'">'
			   					+'<th style="text-align: center;">活动</th>'
			   					+'<th style="text-align: center;" colspan="3">活动'+obj[k].TXN_DETAIL+'</th>'
			   					+'<td style="text-align: center;"><img src="${ctx}/images/frame_remove.png" title="删除" style="cursor:pointer;" onclick="sysAdmin_agentunit_00312_delFun('+obj[k].HUCID+','+id+','+obj[k].TXN_DETAIL+')"/></td>'
			   					+'<input type="hidden" name="machineType" value="1"/>'
			   					+'<input type="hidden" name="txnType" value="1"/>'
			   				...
			   				+'</tr>';*/
								...
								html=html+hb_html+'</tr>';
										// +'<td><input name="ysfRate" type="text" class="easyui-validatebox" data-options="validType:\'money\'" value="'+obj[k].YSF_RATE+'" /></td>'
										// +'<td><input name="debitRate" type="text" class="easyui-validatebox" data-options="validType:\'money\'" value="'+obj[k].DEBIT_RATE+'" /></td>'
										// +'<td><input name="debitFeeamt" type="text" class="easyui-validatebox" data-options="validType:\'money\'" value="'+obj[k].DEBIT_FEEAMT+'" /></td>'
										// +'</tr>';
		   					$("#00312_ACT_TRebateType").before(html);
		   					// $("#00312TRebateType").before(html);
		   					//添加事件
								// if(obj[k].TXN_DETAIL<=21) {
								// 	$('#00312T' + id + '').find("input[name='debitRate']").bind('blur', function () {
								// 		var _debitRate_value = $(this).val();
								// 		$(this).parent().parent().find("input[name='creditRate']").val(_debitRate_value);
								// 	});
								// }
	...
	//删除已添加
	function sysAdmin_agentunit_00312_delFun(hucid,id,txnDetail){
		$.ajax({
	...
	function sysAdmin_00312_addRebate(){
		$('<div id="sysAdmin_00312_addDialog"/>').dialog({
		    ...
		    buttons:[{
		    	...
		    	iconCls:'icon-ok',
		    	handler:function() {
		    		sysAdmin_00312_edit1();
		    		$('#sysAdmin_00312_addDialog').dialog('destroy');
		    	}			    
			},{
				text:'取消',
				iconCls:'icon-cancel',
	...
	function sysAdmin_00312_edit1(){
		var currentTrList = $("#00312_ACT_TR").children("tr");
...
<form id="sysAdmin_agentunit_editForm" method="post" enctype="multipart/form-data">
	...
	<!-- 创建成本数据隐藏域 -->
	<input type="hidden" name="costData" id="00312_costData"/>
	<input type="hidden" name="flag_00312" id="flag_00312"/>
</form>
<fieldset id="00312_ACT_TAB">
	<legend>活动成本信息</legend>
	<table class="table" id="00312_ACT_TABLE">
		<thead>
		<tr>
		...
		<tbody id="00312_ACT_TR">
		<tr id="00312_ACT_TRebateType">
			<th style="text-align: center;">活动</th>
			<td style="text-align: center;" colspan="9"><a id="" onclick="sysAdmin_00312_addRebate()" class="l-btn" href="javascript:void(0)"><span class="l-btn-left"><span class="l-btn-text icon-add" style="padding-left: 20px;">添加活动成本</span></span></a></td>
		</tr>
		</tbody>
...
<form id="00312_hrtCostForm" method="post">
	<!-- 成本信息修改 -->
