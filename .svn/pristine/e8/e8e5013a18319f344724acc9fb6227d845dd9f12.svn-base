...
$(function(){
	var j=1;
	var allRebateTypeSytTemplateLog = new Array();
	var tempname=$('#tempName_SytTemplateLog').val();
	$.ajax({
		url:'${ctx}/sysAdmin/CheckUnitProfitMicro_QUERYSytprofitmicroLogDetail.action',
		dataType:"json",
		type:"post",
		...
		success:function(data) {
			...
			for(var i=0;i<json.length;i++){
				allRebateTypeSytTemplateLog[allRebateTypeSytTemplateLog.length] = json[i].profitRule.toString();
					var html = '<tr id="tr_SytTemplateLog_'+j+'" >'
					...
					if(json[i].ishuabei==1){
						...
						+'<td><input type="text" name="huabeiRate" style="width:100px;" disabled="disabled" class="easyui-validatebox" data-options="required:true,validType:"spaceValidator" " value="'+json[i].huabeiRate+'"  />%</td>'
						+'<th style="text-align: center;">花呗转账费</th>'
						+'<td><input type="text" name="huabeiFee" style="width:100px;" disabled="disabled" class="easyui-validatebox" data-options="required:true,validType:"spaceValidator" " value="'+json[i].huabeiFee+'"  /></td>'
						+'</tr>';
					}
					...
					+'</tr>'
					+'</tr>';
				$("#SytTemplateLogTRebateType").before(html);
			}
   		}
	});
});
...
<div data-options="region:'north',border:false"
	...
	<div id = "SytTemplateLog_tabs" title="模板成本详情" class="easyui-tabs" >
		<form id="ProfitmicroNextSytTemplateLog" style="padding-left:1%;padding-top:1%" method="post" enctype="multipart/form-data">
			<fieldset>
			  <table class="table">
				 <tr>
				   	<th>模版名称：</th>
				   	<td ><input type="text" name="tempName" id="tempName_SytTemplateLog"  style="width:100px;" class="easyui-validatebox" data-options="required:true,validType:'spaceValidator'" value="<%= tempName20602 %>" readonly="true"/><font color="red">&nbsp;*</font></td>
				 </tr>
			  </table>
			</fieldset>
			<input type="hidden" id="tempnameSytTemplateLog" name="Tempname" />
		</form>
		<fieldset>
			<legend>活动</legend>
			<table class="table" id="table_SytTemplateLog">
				<tbody id="SytTemplateLog_table">
			 		<tr id="SytTemplateLogTRebateType">
						<td style="text-align: center;" colspan="9">
							<!-- <a id="" onclick="sysAdmin_SytTemplateLog_addRebate()" class="l-btn" href="javascript:void(0)"><span class="l-btn-left"><span class="l-btn-text icon-add" style="padding-left: 20px;">添加活动</span></span></a> -->
						</td>
					</tr>
			</tbody>
		   </table>
