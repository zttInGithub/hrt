<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!-- 代理商详情，结算开通、代理商资料、代理商修改查看功能，用该页面 -->
<script type="text/javascript">
	$(function(){
			...
			+'<table class="table">'
				...
				+'<tbody id="10141_tab2_body">'
						...
						+'<tr id="10141T14">'
							...
							+'</td>'
							+'<td style="text-align: center;">'
							+'<input style="text-align: center;width:70px" name="K268D" type="text" readonly="readonly" readonly="readonly"/>'
							+'</td>'
							+'<td style="text-align: center;">'
						...
						+'<tr id="10141T15">'
							...
							+'</td>'
							+'<td style="text-align: center;">'
							+'<input style="text-align: center;width:70px" name="K269D" type="text" readonly="readonly"/>'
							+'</td>'
							+'<td style="text-align: center;">'
							+'<input style="text-align: center;width:70px" type="text" name="K269E" readonly="readonly"/>'
							+'</td>'
						+'</tr>'
						+'<tr id="10141T16">'
							+'<th style="text-align: center;">银联二维码</th>'
						...
						+'<tr id="10141T18">'
							...
							+'</td>'
							+'<td style="text-align: center;">'
							+'<input style="text-align: center;width: 70px" type="text" name="K2612D" readonly="readonly"/>'
							+'</td>'
							+'<td style="text-align: center;">'
							+'<input style="text-align: center;width:70px" type="text" name="K2612E" readonly="readonly"/>'
							+'</td>'
						+'</tr>'
						+'<tr id="10141T19">'
		...
		var fenrun = ''
			...
			+'	</table>'
			+'</fieldset>'
		//if(unno.substring(2,3) == 1 || unno.substring(2,3) == 2){//运营中心或者一代
		//带入成本信息,修改
		$.ajax({
			...
			dataType:"json",
	   		...
	   		success:function(data){
	   			if(data.success){
	   				...
	   				if(keys.length != 0){
	   					$("#10141_hrtCostForm").append(html);
	   					//$("#fenrun_10141").append(fenrun);
	   					$("#10141_hrtCostForm").form('load', obj)
							sysAdmin_10141_queryRebate20190912(buid);
	   				}else{
	   					$("#10141_HAS_ACT").remove();
	   					$("#fenrun_10141").remove();
	   				}
	   				//去加载活动成本
	   				// sysAdmin_10141_queryRebate(buid);
	   			}
	   		}
		});
		//}
	...
	function sysAdmin_10141_queryRebate(buid){
		//加载活动成本
		$.ajax({
	   			...
	   			if(data.success){
	   				...
	   				if(obj.length != 0){
	   					...
	   					for(var k=0;k<obj.length;k++){
	   						var tr_list = $("#10141_tab2_body").children("tr");
		   					var id = tr_list.length;
								var tt = '0';
								if(obj[k].DEBIT_FEEAMT){
									tt=obj[k].DEBIT_FEEAMT
								}
	   						var html = '<tr id="10141T'+id+'">'
	...
	function sysAdmin_10141_queryRebate20190912(buid){
		//加载活动成本
		$.ajax({
			url:'${ctx}/sysAdmin/agentunit_queryHrtUnnoCostSingleRebate.action',
			dataType:"json",
			...
			success:function(data){
				if(data.success){
					...
					if(obj.length != 0){
						for(var k=0;k<obj.length;k++){
							var tr_list = $("#10141_ACT_TR").children("tr");
							var id = tr_list.length;

							var html = '<tr id="10141_ACTT'+id+'">'
							...
							html=html+hbHtml+ '</tr>'
							$("#10141_ACT_TRebateType").before(html);
						}
					}
				}
			}
	...
	<fieldset>
		...
		<table class="table">
			<tr>
	    		...
	    		<th>归属机构号：</th>
	    		<td><input type="text" style="width:200px;" name="parentUnno" readonly="readonly"/>
	    		</td>
			</tr>
			<tr>
				<th>代理商级别：</th>
...
<fieldset id="10141_HAS_ACT">
	<legend>活动成本信息</legend>
	<table class="table" id="10141_ACT_TABLE">
		<thead>
		<tr>
		...
		<tbody id="10141_ACT_TR">
		<tr id="10141_ACT_TRebateType"></tr>
		</tbody>
	</table>
</fieldset>
<form id="10141_hrtCostForm" method="post">
