...
<script type="text/javascript">
	$(function() {
		$('#sysAdmin_merchantMicroZKPlus_datagrid').datagrid({
			url :'${ctx}/sysAdmin/merchant_listMerchantMicroInfoZKPlus.action',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [[{
				...
				formatter : function(value,row,index) {
					if(row.approveStatus == 'C'){
						...
						return '<img src="${ctx}/images/frame_pencil.png" title="修改商户" style="cursor:pointer;" onclick="sysAdmin_merchantMicro12Plus_editFun('+index+','+row.isForeign+','+row.merchantType+','+row.bmid+','+row.accType+')"/>&nbsp;&nbsp'+
	...
	function sysAdmin_merchantMicro12Plus_editFun(index,isForeign,merchantType,bmid,accType){
		$('<div id="sysAdmin_merchantMicro_editDialog"/>').dialog({
		    ...
		    modal: true,
		    onLoad:function() {
		    	var rows = $('#sysAdmin_merchantMicroZKPlus_datagrid').datagrid('getRows');
				var row = rows[index];
					...
					$('#sysAdmin_merchantMicro_deditForm').form('submit', {
						...
						success:function(data) {
							...
							if (result.sessionExpire) {
				    			...
				    			if (result.success) {
				    				$('#sysAdmin_merchantMicroZKPlus_datagrid').datagrid('reload');
	...
	//表单查询
	function sysAdmin_merchantMicroZKPlus_searchFun10() {
		$('#sysAdmin_merchantMicroZKPlus_datagrid').datagrid('load', serializeObject($('#sysAdmin_merchantMicroZK_searchPlusForm'))); 
	}

	//清除表单内容
	function sysAdmin_merchantMicroZKPlus_cleanFun10() {
...
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:80px; overflow: hidden; padding-top:10px;">
			...
			<table class="tableForm" >
				...
				<tr>
					<td colspan="5" style="text-align: center;">
						...
						onclick="sysAdmin_merchantMicroZKPlus_searchFun10();">查询</a> &nbsp;
						<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" 
						onclick="sysAdmin_merchantMicroZKPlus_cleanFun10();">清空</a>	
					</td>
				</tr>
			</table>
		</form>
 	...
 	<div data-options="region:'center', border:false" style="overflow: hidden;">  
		<table id="sysAdmin_merchantMicroZKPlus_datagrid" style="overflow: hidden;"></table> 
	</div>
</div>
