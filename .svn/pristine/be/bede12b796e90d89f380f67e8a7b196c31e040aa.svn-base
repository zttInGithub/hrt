 ...
 <script type="text/javascript">
	...
	function check_search(){
	  ...
	  if(((end-start)/(24*60*60*1000))>31 || txnDay.substring(0,7)!=txnDay1.substring(0,7)){
			...
			var url = "${ctx}/phone/phoneWechatPublicAcc_queryMySettlement.action?mid="+$("#xmh_mid_20130").val()+"&txnDay="+txnDay+"&txnDay1="+txnDay1+"&prod="+prod+"&openid="+$("#openid").val();
			var dataArray=0;
			...
			function mycard_load(data) {
			    ...
			    $.each(dataArray, function (i, json) {
			    	if(json.sum!=undefined&&json.sumAmount!=undefined){
			        	sum = json.sum;
			        	sumMDA = json.sumAmount;
			        }
					var rownum=json.FIID;
					var CDATE=json.CDATE;
					var TXNAMT=json.TXNAMT;
					var SMAMOUNT=json.SMAMOUNT;
					html1.append("<table><tr><td class=\"col-xs-3\" >"+CDATE+"</td><td class=\"col-xs-4\">"+TXNAMT+"</td><td class=\"col-xs-4\">"+SMAMOUNT+"</td><td class=\"col-xs-5\">"+rownum+"</td></tr></table>");
			    });
    ...
    <div class="transaction-query">
		...
		<input type="hidden" id="page" name="page" value = 1 />
			<div class="transaction-date">
				...
				<select class="form-control" id="weChat_prod" name="prod" style="width: auto;">
...
<%--					<option value="10005">会员宝收银台</option>--%>
                ...
                <input id="date-end" name="txnDay1" type="date" placeholder="2017-01-01"  style="margin-left: 2%;width: 30%" onchange="closeTxn1();"/>
           	 	</div>
	...
	<!-- 交易内容显示-->
        ...
        <div id = "sum" class= "mysum">
            ...
            <table style="margin-top: 0.2rem">
                <thead class="row">
                <tr>
                	<th class="col-xs-3"><p>交易日期</p></th>
                	<th class="col-xs-4"><p>交易额<span class="yuan">(元)</span></p></th>
                	<th class="col-xs-4"><p>结算额<span class="yuan">(元)</span></p></th>
                    <th class="col-xs-5"><p>交易类型</p></th>
                </tr>
                </thead>
            </table>
