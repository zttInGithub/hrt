...
<script type="text/javascript">
	...
	$(function() {
		$('#sysAdmin_merchantjoin_datagrid').datagrid({
			url :'${ctx}/sysAdmin/merchant_listMerchantInfoWJoin.action',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function sysAdmin_merchantjoin_queryEditFun(bmid){
		var isM35 = $("#sysAdmin_merchantList_queryForm #isM35").combobox("getValue");
		var accType = $("#sysAdmin_merchantList_queryForm input[name='accType']:checked").val();
		var conMccid = $("#sysAdmin_merchantList_queryForm #conMccid").combobox("getValue");
		$('<div id="sysAdmin_merchantjoin_editDialog"/>').dialog({
			title: '<span style="color:#157FCC;">确认</span>',
			width: 380,   
		    height: 240, 
			...
			buttons:[{
				...
				handler:function() {
		    		...
		    		$('#sysAdmin_merchantjoin_editForm').form('submit', {
						url:'${ctx}/sysAdmin/merchant_updateMerchantInfoC.action?bmids='+bmid+'&isM35='+isM35+'&accType='+accType+'&conMccid='+conMccid,
						success:function(data) {
							$.messager.progress('close');
							var result = $.parseJSON(data);
							if (result.sessionExpire) {
