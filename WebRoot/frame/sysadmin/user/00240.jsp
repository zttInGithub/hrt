...
<script type="text/javascript">
	...
	//初始化datagrid
	$(function() {
		$('#sysAdmin_user_datagrid').datagrid({	
			url : '${ctx}/sysAdmin/user_listUsers.action',
			fit:true,
			fitColumns : true,
			border : false,
			nowrap : true,
	...
	function sysAdmin_user_searchFun() {
		$('#sysAdmin_user_datagrid').datagrid('load', serializeObject($('#sysAdmin_user_searchForm')));
	}
					...
					$('#sysAdmin_userAdd_addForm').form('submit', {
					    ...
					    success:function(data){   
							...
							if (res.sessionExpire) {
								...
								if(res.success) {
									$('#sysAdmin_user_datagrid').datagrid('unselectAll');
									$('#sysAdmin_user_datagrid').datagrid('reload');
									dia.dialog('destroy');
									$.messager.show({
										title : '提示',
										msg : res.msg
	...
	function sysAdmin_user_editUserFun(index) {
		...
		$('<div id="sysAdmin_user_edit"/>').dialog({
		    ...
		    modal: true,
		    onLoad: function() {
		    	var rows = $('#sysAdmin_user_datagrid').datagrid('getRows');
				var row = rows[index];
...
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:90px; overflow: hidden; padding-top:10px;">
		<form id="sysAdmin_user_searchForm" style="padding-left:5%">
			<table class="tableForm" >
				<tr>
					...
					<th>&nbsp;&nbsp;&nbsp;&nbsp;用户级别</th>
					<td><select name="useLvl" style="width: 100px;" >
