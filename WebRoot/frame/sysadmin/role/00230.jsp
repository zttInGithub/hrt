...
<script type="text/javascript">
	...
	//初始化datagrid
	$(function() {
		$('#sysAdmin_role_datagrid').datagrid({
			url :'${ctx}/sysAdmin/role_listRoles.action',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [[{
				...
				align : 'center',
				formatter : function(value,row,index) {
					return '<img src="${ctx}/images/frame_pencil.png" title="修改" style="cursor:pointer;" onclick="sysAdmin_role_editFun('+index+')"/>&nbsp;&nbsp'+
					'<img src="${ctx}/images/frame_remove.png" title="删除" style="cursor:pointer;" onclick="sysAdmin_role_delFun('+row.roleID+')"/>&nbsp;&nbsp'+
		    		...
		    		$('#sysAdmin_role_addForm').form('submit',{
		    			...
		    			success:function(data) {
			    			...
			    			if (result.sessionExpire) {
				    			...
				    			if (result.success) {
				    				$('#sysAdmin_role_datagrid').datagrid('reload');
	...
	function sysAdmin_role_editFun(index) {
		$('<div id="sysAdmin_role_editDialog"/>').dialog({
		    ...
		    modal: true,
		    onLoad:function() {
		    	var rows = $('#sysAdmin_role_datagrid').datagrid('getRows');
				var row = rows[index];
...
<table id="sysAdmin_role_datagrid" style="overflow: hidden;"></table>
