...
<script type="text/javascript">
	//初始化datagrid
	$(function() {
		$('#sysAdmin_merchantMicroZK_datagrid').datagrid({
			url :'${ctx}/sysAdmin/merchant_listMerchantMicroInfoZK.action',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [[{
				...
				formatter : function(value,row,index) {
					if(row.approveStatus == 'C'){
						...
						//return '<img src="${ctx}/images/start.png" title="修改终端" style="cursor:pointer;" onclick="sysAdmin_merchantMicro_terminalinfo_queryFun('+index+')"/>&nbsp;&nbsp';
						//'<img src="${ctx}/images/frame_pencil.png" title="修改商户" style="cursor:pointer;" onclick="sysAdmin_merchantMicro12_editFun('+index+','+row.isForeign+','+row.merchantType+','+row.bmid+','+row.accType+')"/>';
					}else{
						return '<img src="${ctx}/images/frame_pencil.png" title="修改商户" style="cursor:pointer;" onclick="sysAdmin_merchantMicro12_editFun('+index+','+row.isForeign+','+row.merchantType+','+row.bmid+','+row.accType+')"/>&nbsp;&nbsp'+
							'<img src="${ctx}/images/frame_remove.png" title="删除商户" style="cursor:pointer;" onclick="sysAdmin_merchantMicro10_delFun('+row.bmid+')"/>';
					}
				}
			}]]
		});
	...
	function sysAdmin_merchantMicro12_editFun(index,isForeign,merchantType,bmid,accType){
		$('<div id="sysAdmin_merchantMicro_editDialog"/>').dialog({
		    ...
		    modal: true,
		    onLoad:function() {
		    	var rows = $('#sysAdmin_merchantMicroZK_datagrid').datagrid('getRows');
				var row = rows[index];
	...
	function sysAdmin_merchantMicro10_delFun(bmid){
	...
	//申请终端
	function sysAdmin_merchantMicro_terminalinfo_queryFun(index){
	   	var rows = $('#sysAdmin_merchantMicroZK_datagrid').datagrid('getRows');
		var row = rows[index];
		var bmid=row.bmid;
		var unno=row.unno;
	...
	function sysAdmin_merchantMicroZK_searchFun10() {
		$('#sysAdmin_merchantMicroZK_datagrid').datagrid('load', serializeObject($('#sysAdmin_merchantMicroZK_searchForm'))); 
	}

	//清除表单内容
	function sysAdmin_merchantMicroZK_cleanFun10() {
		$('#sysAdmin_merchantMicroZK_searchForm input').val('');
	}
</script>
...
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:80px; overflow: hidden; padding-top:10px;">
		<form id="sysAdmin_merchantMicroZK_searchForm" style="padding-left:10%">
 	...
 	<div data-options="region:'center', border:false" style="overflow: hidden;">  
		<table id="sysAdmin_merchantMicroZK_datagrid" style="overflow: hidden;"></table> 
	</div>
</div>
