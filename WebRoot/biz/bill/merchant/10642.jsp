...
/*
	...
	*/
	...
	$(function(){
		...
		$.ajax({
			url:'${ctx}/sysAdmin/merchantAuthenticity_serachMerAuthInfoDetailed.action',
    		dataType:"json",  
    		type:"post",
    		data:{bmatid:bmatid},
	...
	function showBigImg(img){
		...
		$('#sysAdmin_10642_editForm').after(imgDialog);
		$("#img").attr("src",img);
...
<form id="sysAdmin_10642_editForm" method="post">
	<fieldset>
		<legend>认证信息</legend>
		<table class="table">
			<tr>
	    		<th>APP用户名：</th>
	    		<td><input type="text" name="username" style="width:200px;" readonly="readonly" class="easyui-validatebox" data-options="required:true" maxlength="100"/><font color="red">&nbsp;</font></td>
	    	...
	    	<tr>
	    		...
	    		<th>认证返回信息：</th>
	    		<td><input type="text" name="respinfo" style="width:200px;" readonly="readonly" class="easyui-validatebox" maxlength="20"/><font color="red">&nbsp;</font></td>
	    	</tr>
		</table>
	</fieldset>
			...
			<tr>
	    		...
	    		<td>
	    			<select id="approveNote" name="approveNote" style="width:180px;height:25px" class="easyui-validatebox" data-options="required:true">
	    					<option value="照片信息不符" selected="selected" >照片信息不符</option>
	    					<option value="姓名输入有误">姓名输入有误</option>
	    					<option value="身份证信息录入错误">身份证信息录入错误</option>
	    					<option value="卡片不是交易卡片">卡片不是交易卡片</option>
	    					<option value="银行卡照片未上传">银行卡照片未上传</option>
	    					<option value="身份证照片未上传">身份证照片未上传</option>
	    					<option value="请上传银行卡和身份证原件的正面照片">请上传银行卡和身份证原件的正面照片</option>
	    					<option value="照片内容不清晰,请重新上传">照片内容不清晰,请重新上传</option>
	    					<option value="商户涉及风险，未审核通过">商户涉及风险，未审核通过</option>
	    			</select>
	    			<font color="red">&nbsp;*</font>
	    		</td>
	    		<th></th><td></td>
