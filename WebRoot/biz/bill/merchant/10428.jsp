...
<script type="text/javascript">
	//初始化datagrid
	$(function() {
		$('#sysAdmin_merchantZK_datagrid_FG').datagrid({
		    		...
		    		$('#sysAdmin_merchant11_addForm').form('submit',{
		    			...
		    			success:function(data) {
			    			...
			    			if (result.sessionExpire) {
				    			...
				    			if (result.success) {
				    				$('#sysAdmin_merchantZK_datagrid_FG').datagrid('reload');
	...
	function sysAdmin_merchant12_addFun(){
		$.messager.confirm('继续申请','尊敬的用户：通过 小微商户入网 报单的商户，单日累计交易限额为5万元，如不能满足您的使用需求，请选择其他入口。',function(result) {
			if (result) {
				$('<div id="sysAdmin_addMerchant12_datagrid"/>').dialog({
					title: '<span style="color:#157FCC;">小微商户签约</span>', 
					width: 1000,    
				    height: 600,  
				    resizable:true,
		    		maximizable:true,
				    ...
				    buttons:[{
				    	...
				    	iconCls:'icon-ok',
				    	handler:function() {
				    	if($('#materialUpLoad3File').val()!="" && $('#materialUpLoad4File').val()!="" && $('#materialUpLoad5File').val()!="" && $('#registryUpLoadFile').val()!="" && $('#photoUpLoadFile').val()!="" && $('#bigdealUpLoadFile').val()!="" && $('#materialUpLoad2File').val()!=""&&$('#posBackImgFile').val()!=""){
	...
	function sysAdmin_merchant13_addFun(){
		$.messager.confirm('继续申请','尊敬的用户：通过“企业商户入网”接口报单的商户，需提供企业（单位）营业证照，如不能提供，请选择其他入口。',function(result) {
		  	if (result) {
				$('<div id="sysAdmin_addMerchant13_datagrid"/>').dialog({
					title: '<span style="color:#157FCC;">企业商户签约</span>', 
					width: 1000,    
				    height: 600,  
				    resizable:true,
		    		maximizable:true,
				    closed: false,
				    href: '${ctx}/biz/bill/merchant/10425.jsp',  
				    modal: true,
				    buttons:[{
				    	text:'确认',
				    	iconCls:'icon-ok',
				    	handler:function() {
				    	var accType_10428 = $("input[name='accType']:checked").val();
				    	if(accType_10428==1||$('#materialUpLoad1File').val()!=""){
				    	}else{
				    		$.messager.alert('提示', '请上传必要的文件！');
				    		return;
				    	}
				    	if($('#registryUpLoadFile').val()!="" && $('#bigdealUpLoadFile').val()!=""&& $('#photoUpLoadFile').val()!="" && $('#bupLoadFile').val()!="" && $('#legalUploadFile').val()!="" && $('#materialUpLoadFile').val()!="" && $('#materialUpLoad3File').val()!="" && $('#materialUpLoad4File').val()!="" && $('#materialUpLoad5File').val()!="" && $('#rupLoadFile').val()!=""&&$('#posBackImgFile').val()!=""){
			...
			if (result) {
				$('<div id="sysAdmin_addMerchant14_datagrid"/>').dialog({
				    ...
				    buttons:[{
				    	...
				    	iconCls:'icon-ok',
				    	handler:function() {
				    	if($('#registryUpLoadFile').val()!="" && $('#bigdealUpLoadFile').val()!=""&& $('#photoUpLoadFile').val()!="" && $('#bupLoadFile').val()!="" && $('#legalUploadFile').val()!="" && $('#materialUpLoadFile').val()!="" && $('#materialUpLoad3File').val()!="" && $('#materialUpLoad4File').val()!="" && $('#materialUpLoad5File').val()!="" && $('#rupLoadFile').val()!="" && $('#materialUpLoad6File').val()!=""){
				    		$.messager.confirm('提示','请确认信息是否无误，提交申请后将不能调整！',function(re) {
				    			if (re) {
			...
			if (result) {
				$('<div id="sysAdmin_addMerchant15_datagrid"/>').dialog({
				    ...
				    buttons:[{
				    	...
				    	iconCls:'icon-ok',
				    	handler:function() {
				    	if($('#registryUpLoadFile').val()!="" && $('#bigdealUpLoadFile').val()!=""&& $('#photoUpLoadFile').val()!="" && $('#bupLoadFile').val()!="" && $('#legalUploadFile').val()!="" && $('#materialUpLoadFile').val()!="" && $('#materialUpLoad3File').val()!="" && $('#materialUpLoad4File').val()!="" && $('#materialUpLoad5File').val()!="" && $('#rupLoadFile').val()!="" && $('#materialUpLoad6File').val()!="" && $('#materialUpLoad7File').val()!=""){
				    		$.messager.confirm('提示','请确认信息是否无误，提交申请后将不能调整！',function(re) {
				    			if (re) {
	...
	**/
	function sysAdmin_merchant12_editFun(index,isForeign,merchantType,bmid,accType){
		$('<div id="sysAdmin_merchant_editDialog"/>').dialog({
		    ...
		    modal: true,
		    onLoad:function() {
		    	var rows = $('#sysAdmin_merchantZK_datagrid_FG').datagrid('getRows');
				var row = rows[index];
	...
	function sysAdmin_merchantterminalinfo_queryFun(index){
		...
		$('<div id="sysAdmin_merchantterminalinfo_run"/>').dialog({
		    ...
		    height: 550,   
		    closed: false,
		    href:'${ctx}/biz/bill/merchant/10491.jsp?bmid='+bmid+'&unno='+unno+'&mid='+mid+'&baddr='+encodeURIComponent(encodeURIComponent(baddr))+'&contactPerson='+encodeURI(encodeURI(contactPerson))+'&contactPhone='+contactPhone+'&contactTel='+contactTel,  
		    modal: true, 
			onClose : function() {
				$(this).dialog('destroy');
			}  
	...
	function sysAdmin_merchantZK_searchFun10() {
		$('#sysAdmin_merchantZK_datagrid_FG').datagrid('load', serializeObject($('#sysAdmin_merchantZK_searchForm'))); 
	}

	//清除表单内容
	function sysAdmin_merchantZK_cleanFun10() {
		$('#sysAdmin_merchantZK_searchForm input').val('');
	}
</script>
...
<div class="easyui-layout" data-options="fit:true, border:false">
	<div data-options="region:'north',border:false" style="height:80px; overflow: hidden; padding-top:25px;">
		<form id="sysAdmin_merchantZK_searchForm" style="padding-left:3%">
 	...
 	<div data-options="region:'center', border:false" style="overflow: hidden;">  
		<table id="sysAdmin_merchantZK_datagrid_FG" style="overflow: hidden;"></table> 
	</div>
</div>
